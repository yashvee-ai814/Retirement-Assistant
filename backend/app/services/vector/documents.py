...
from .client import get_chroma_client, COLLECTION_NAME
from .ingest import DOCS_DIR, ingest_pdf_file
from ..shared.models import DocumentInfo
...
async def list_documents(db: AsyncSession = Depends(get_db)) -> list[DocumentInfo]:
    ...
    return [doc_to_info(d) for d in result.scalars().all()]
